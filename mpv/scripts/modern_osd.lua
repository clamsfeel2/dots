...
-- Function to prompt the user for input using rofi
local function prompt_for_input(prompt)
    local handle = io.popen('rofi -dmenu -p "' .. prompt .. '"')
    local input = handle:read("*a"):gsub("\n", "")  -- Remove trailing newline
    handle:close()
    return input
end
...
local user_opts = {
    ...
    windowcontrols = 'no',        -- whether to show OSC window controls, 'auto', 'yes' or 'no'
    ...
    scalefullscreen = 2.0,          -- scaling of the controller when fullscreen
    scaleforcedwindow = 2.0,        -- scaling when rendered on a forced window

    -- interface settings --
    hidetimeout = 3000,             -- duration in ms until OSC hides if no mouse movement
    ...
    showontop = false,               -- show window on top button
    showinfo = true,               -- show the info button
    downloadbutton = true,          -- show download button for web videos
    downloadpath = "~/Videos/Downloaded/", -- the download path for videos
    showyoutubecomments = false,    -- EXPERIMENTAL - not ready
    commentsdownloadpath = "~/Videos/Downloaded/", -- the download path for the comment JSON file
	...
	ytdlpQuality = '-f bestvideo+bestaudio/best'
        ...
        function ()
            if (not state.videoCantBeDownloaded) then
                ...
                elseif state.downloading then
					...
					local filename = prompt_for_input("Enter filename: ")
					local command = {
						"yt-dlp",
                        user_opts.ytdlpQuality,
						"--recode-video", "mkv",  -- convert video format to MKV
                        ...
                        "-P " .. localpathnormal,
						state.path  -- URL of the video to download
					}
